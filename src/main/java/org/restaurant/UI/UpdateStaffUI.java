...
public class UpdateStaffUI {
    ...
    public UpdateStaffUI(Integer id){
        ...
        addBtn.addActionListener(b->{
            ...
            if(email.equalsIgnoreCase(userr.getEmail())){
                flag = true;
            }

            if(flag){
                staffAuthenticationService.updateAllValues(id, name, email, pass, type);

                frame.dispose();
                new StaffUI();
            }else {
                JOptionPane.showMessageDialog(frame, "Email Already Taken!!!Try changing email!!!");
            }
